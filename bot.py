#!/usr/bin/env python3
"""
CoinDCX Ultimate Production Bot – FINAL VERSION
...
# ================================
#  FIXED: COINDCX DATA FETCH WITH ROBUST PAIR MATCHING & RETRIES
# ================================
...
def get_markets():
    ...
    Cached for 1 hour. Retries up to 3 times on failure.
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                if isinstance(data, list) and data:
                    ...
                    logger.warning(f"Markets response empty/invalid (attempt {attempt})")
            else:
...
def get_pair_for_symbol(symbol):
    ...
    markets = get_markets()
    if not markets:
        # Fallback to hardcoded list for major coins
        fallback = {
            "BTCINR": "B-BTC_INR",
            "ETHINR": "B-ETH_INR",
            "XRPINR": "B-XRP_INR",
        ...
        if symbol in fallback:
            ...
            # Log only once per hour per symbol to avoid spam
            key = f"pair_{symbol}"
            now = time.time()
            if key not in last_error_logged or now - last_error_logged[key] > 3600:
                logger.error(f"Could not find pair for {symbol} (no markets and no fallback)")
    ...
    if not matching:
        ...
        now = time.time()
        if key not in last_error_logged or now - last_error_logged[key] > 3600:
            logger.error(f"No matching pair for {symbol} in markets list")
            last_error_logged[key] = now
        return None
...
def get_current_price(symbol):
    ...
    for attempt in range(1, 4):
        try:
            # CoinDCX ticker endpoint returns all tickers; we filter.
...
def process_command(text):
    ...
    # ... (include all other commands from your original bot) ...
    # For brevity, only essential ones are shown; add your full list.
