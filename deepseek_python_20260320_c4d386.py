#!/usr/bin/env python3
"""
CoinDCX Ultimate Production Bot – FINAL VERSION with All Stability Fixes
...
- Max drawdown stop (50%), max 3 concurrent trades
- Detailed logging
"""
...
MAX_CONCURRENT_TRADES = 3        # NEW: global trade limit
...
# ================================
#  DATABASE (unchanged, but included for completeness)
# ================================
def init_db():
    ...
    default_user_settings = [
        ...
        ("max_concurrent_trades", str(MAX_CONCURRENT_TRADES)),  # NEW
    ]
...
def record_signal(coin, price, signal, confidence, grade, grade_score, leverage, position_size):
    with db_lock:
        ...
        c.execute('''INSERT INTO signals
                     ...
                     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)''',
                  (coin, datetime.utcnow(), price, signal, confidence, grade, grade_score, leverage, position_size))
        db_conn.commit()
        return c.lastrowid
...
def update_signal_outcome(sig_id, outcome, pnl=0.0):
    with db_lock:
        ...
        c.execute('''UPDATE signals SET outcome = ?, pnl = ?, evaluated = 1, evaluated_at = ?
                     WHERE id = ?''', (outcome, pnl, datetime.utcnow(), sig_id))
        db_conn.commit()
...
def add_open_trade(coin, entry_price, quantity):
    with db_lock:
        ...
        c.execute("INSERT INTO open_trades (coin, entry_price, quantity, entry_time) VALUES (?, ?, ?, ?)",
                  (coin, entry_price, quantity, datetime.utcnow()))
        db_conn.commit()
...
def get_markets():
    ...
    now = time.time()
    if now - markets_cache["timestamp"] < 3600 and markets_cache["data"]:
        return markets_cache["data"]
    try:
        resp = requests.get(f"{COINDZX_BASE}/exchange/v1/markets", timeout=10)
        markets = resp.json()
        markets_cache = {"timestamp": now, "data": markets}
        return markets
    except Exception as e:
        logging.error(f"Failed to fetch markets: {e}")
        if "markets" not in last_error_sent or time.time() - last_error_sent["markets"] > 3600:
            send_message("⚠️ CoinDCX markets API error. Check logs.")
            last_error_sent["markets"] = time.time()
        return markets_cache["data"] or []

def get_pair_for_symbol(symbol):
    cached = get_cached_pair(symbol)
    if cached:
        return cached
    markets = get_markets()
    for pair in markets:
        if pair.endswith(symbol.replace("INR", "_INR")):
            cache_pair(symbol, pair)
            return pair
    logging.error(f"Could not find pair for {symbol}")
    return None
...
def get_ohlcv(symbol, interval, limit=200):
    ...
    try:
        ...
        logging.error(f"CoinDCX fetch error for {symbol} {interval}: {e}")
        key = f"{symbol}_{interval}"
        if key not in last_error_sent or time.time() - last_error_sent[key] > 3600:
            send_message(f"⚠️ Data error for {symbol} ({interval}). Check logs.")
            last_error_sent[key] = time.time()
        return None
...
def get_current_price(symbol):
    ...
    try:
        ...
        return float(ticker.get(pair, {}).get('last_price', 0))
    except Exception as e:
...
# ================================
#  BTC TREND FILTER (using EMA200 series)
# ================================
def get_btc_trend():
...
def get_btc_dominance():
    ...
    try:
        ...
        logging.error(f"Dominance fetch error: {e}")
        return None
...
# ================================
#  ADVANCED ANALYSIS FUNCTION (with leverage safety)
# ================================
def analyze_coin_mtf(symbol):
    ...
    if current_candle_size > FAKE_BREAKOUT_MULTIPLIER * avg_candle_size:
        logging.info(f"{symbol} fake breakout: candle too large")
        return None
    ...
    if atr_percent > max_atr_percent:
        logging.info(f"{symbol} ATR% {atr_percent:.2f}% > {max_atr_percent}% – skipping")
        return None

    # Market regime filter
    if adx_val < MIN_ADX_FOR_TRADE:
        logging.info(f"{symbol} ADX {adx_val:.1f} < {MIN_ADX_FOR_TRADE} – ranging market, skip")
        return None
    ...
    # NEW: Leverage safety filter
    if atr_percent > LEVERAGE_ATR_THRESHOLD:
        leverage = min(leverage, 3)
        logging.info(f"{symbol} ATR% {atr_percent:.1f} > {LEVERAGE_ATR_THRESHOLD} – leverage capped to {leverage}")
    ...
    if use_btc and signal_main in ("BUY", "SELL"):
        ...
        if btc_trend:
            if signal_main == "BUY" and btc_trend == "bearish":
                logging.info(f"{symbol} BUY skipped: BTC trend bearish")
                return None
            if signal_main == "SELL" and btc_trend == "bullish":
                logging.info(f"{symbol} SELL skipped: BTC trend bullish")
                return None
    ...
    if use_dom and signal_main in ("BUY", "SELL"):
        ...
        if dom and dom > DOMINANCE_THRESHOLD and symbol != BTC_SYMBOL:
            logging.info(f"{symbol} skipped: BTC dominance {dom:.1f}% > {DOMINANCE_THRESHOLD}%")
            return None

    use_news = get_user_setting("use_news_filter", "false") == "true"
    if use_news and signal_main in ("BUY", "SELL"):
        if not get_news_sentiment():
            logging.info(f"{symbol} skipped due to negative news")
            return None

    # Position size with safety (FIXED formula)
    ...
    if capital > 0 and sl is not None:
        ...
        if position_size_inr < min_pos:
            logging.info(f"{symbol} position size ₹{position_size_inr:.2f} < min ₹{min_pos}, skipping")
            return None
...
# ================================
#  FINAL BACKTESTING MODULE (with all fixes)
# ================================
def backtest(symbol, timeframe, days=30):
    """
    Run backtest on last 'days' of data using the exact live strategy.
    Returns dict with performance metrics.
    """
    # Map timeframe to minutes
    tf_map = {"1m": 1, "5m": 5, "15m": 15, "30m": 30, "1h": 60, "4h": 240, "1d": 1440}
    main_minutes = tf_map.get(timeframe, 15)
    confirm_tf = "1h"
    confirm_minutes = 60

    # Calculate number of candles needed
    main_limit = int((days * 24 * 60) / main_minutes) + 500
    confirm_limit = int((days * 24 * 60) / confirm_minutes) + 100

    # Fetch main and confirmation data
    main_ohlcv = get_ohlcv(symbol, timeframe, limit=main_limit)
    confirm_ohlcv = get_ohlcv(symbol, confirm_tf, limit=confirm_limit)
    if not main_ohlcv or not confirm_ohlcv or len(main_ohlcv) < 250 or len(confirm_ohlcv) < 50:
        return None

    # Convert to lists for speed
    main_closes = [c['close'] for c in main_ohlcv]
    main_highs = [c['high'] for c in main_ohlcv]
    main_lows = [c['low'] for c in main_ohlcv]
    main_volumes = [c['volume'] for c in main_ohlcv]

    confirm_closes = [c['close'] for c in confirm_ohlcv]
    confirm_highs = [c['high'] for c in confirm_ohlcv]
    confirm_lows = [c['low'] for c in confirm_ohlcv]

    n_main = len(main_ohlcv)
    n_confirm = len(confirm_ohlcv)
    candles_per_confirm = confirm_minutes // main_minutes

    # Precompute EMA200 series
    ema200_main_series = ema_series(main_closes, 200)
    ema200_confirm_series = ema_series(confirm_closes, 200)

    # Parameters (same as live)
    sl_mult = get_param("atr_multiplier_sl")
    tp_mult = get_param("atr_multiplier_tp")
    profit_target = get_param("profit_target_percent") / 100
    holding_period_candles = int(get_param("holding_period_minutes") / main_minutes)
    ...
    btc_trends = []
    if use_btc:
        btc_ohlcv = get_ohlcv(BTC_SYMBOL, "1h", limit=confirm_limit)
        if btc_ohlcv and len(btc_ohlcv) > 200:
            btc_closes = [c['close'] for c in btc_ohlcv]
            btc_ema200_series = ema_series(btc_closes, 200)
            btc_trends = ["bullish" if btc_closes[i] > btc_ema200_series[i] else "bearish" for i in range(len(btc_closes))]

    # Trading limits
    initial_capital = 100000.0
    capital = initial_capital
    risk_percent = 1.0
    max_trades_per_hour = int(get_user_setting("max_trades_per_hour", "3"))
    max_exposure_percent = float(get_user_setting("max_global_exposure_percent", "20"))
    max_concurrent_trades = int(get_user_setting("max_concurrent_trades", str(MAX_CONCURRENT_TRADES)))
    cooldown_candles = int(COOLDOWN_HOURS * 60 / main_minutes)
    fee_percent = 0.001
    slippage = 0.0005
    ...
    def get_confirm_idx(main_idx):
        return main_idx // candles_per_confirm

    # Main loop
    start_idx = 200
    for i in range(start_idx, n_main):
        # Max drawdown stop
        if capital < initial_capital * (1 - MAX_DRAWDOWN_PERCENT / 100):
            logging.info(f"Max drawdown {MAX_DRAWDOWN_PERCENT}% reached, stopping backtest")
            break

        # Update equity curve (mark to market of open trades)
        current_equity = capital
        for ot in open_trades:
            if ot['signal'] == "BUY":
                unrealized = (main_closes[i] - ot['entry_price']) * ot['remaining_quantity']
            else:
                unrealized = (ot['entry_price'] - main_closes[i]) * ot['remaining_quantity']
            current_equity += unrealized
        equity_curve.append(current_equity)

        # Check open trades for exit (including trailing stop and partial)
        to_remove = []
        for ot in open_trades:
            if ot['remaining_quantity'] <= 0:
                continue

            candles_held = i - ot['entry_index']
            entry_price = ot['entry_price']
            signal = ot['signal']
            original_sl = ot['sl']
            atr_val = ot.get('atr', 0)
            current_price = main_closes[i]

            # Calculate R multiple
            if signal == "BUY":
                r_multiple = (current_price - entry_price) / (entry_price - original_sl) if entry_price != original_sl else 0
            else:
                r_multiple = (entry_price - current_price) / (original_sl - entry_price) if original_sl != entry_price else 0

            # Partial exit at 1R (only once)
            if r_multiple >= PARTIAL_EXIT_R_MULTIPLIER and not ot.get('partial_done', False):
                close_qty = ot['remaining_quantity'] * PARTIAL_EXIT_PERCENT
                ...
                entry_fee_part = entry_price * close_qty * fee_percent
                exit_fee_part = exit_price * close_qty * fee_percent
                net_pnl = gross_pnl - entry_fee_part - exit_fee_part

                capital += net_pnl
                ot['remaining_quantity'] -= close_qty
                ...
                logging.info(f"Partial exit at candle {i}: {signal} {symbol} qty {close_qty:.4f}, PnL {net_pnl:.2f}")
            ...
            # Check trailing stop activation (after partial)
            if ot.get('partial_done', False) and not ot.get('trailing_activated', False):
                # Activate trailing after partial
                ot['trailing_activated'] = True
                logging.info(f"Trailing activated for {symbol} at candle {i}")

            # Update trailing stop
            if ot.get('trailing_activated', False):
                if signal == "BUY":
                    new_sl = current_price - atr_val * TRAIL_STOP_ATR_MULTIPLIER
                    if new_sl > ot['sl']:
                        ot['sl'] = new_sl
                        logging.info(f"Trailing SL updated to {new_sl:.2f} for {symbol}")
                else:
                    new_sl = current_price + atr_val * TRAIL_STOP_ATR_MULTIPLIER
                    if new_sl < ot['sl']:
                        ot['sl'] = new_sl
            ...
            exit_price = None
            if signal == "BUY":
                if main_highs[i] >= ot['tp']:
                    exit_price = ot['tp']
                    hit = True
                    logging.info(f"TP hit for {symbol} at candle {i}")
                elif main_lows[i] <= ot['sl']:
                    exit_price = ot['sl']
                    hit = True
                    logging.info(f"SL hit for {symbol} at candle {i}")
            else:  # SELL
                if main_lows[i] <= ot['tp']:
                    exit_price = ot['tp']
                    hit = True
                    logging.info(f"TP hit for {symbol} at candle {i}")
                elif main_highs[i] >= ot['sl']:
                    exit_price = ot['sl']
                    hit = True
                    logging.info(f"SL hit for {symbol} at candle {i}")
            ...
            if hit:
                ...
                if signal == "BUY":
                    gross_pnl = (exit_price - entry_price) * ot['remaining_quantity']
                else:
                    gross_pnl = (entry_price - exit_price) * ot['remaining_quantity']

                entry_fee = entry_price * ot['remaining_quantity'] * fee_percent
                exit_fee = exit_price * ot['remaining_quantity'] * fee_percent
                ...
                trades.append({
                    ...
                    'exit_price': exit_price,
                    'signal': signal,
                    'quantity': ot['remaining_quantity'],
                    'pnl': net_pnl,
                    'outcome': 'win' if net_pnl > 0 else 'loss',
                    'partial': False
                })
        ...
        # Get data slices for current candle (only up to i)
        main_slice = main_ohlcv[:i+1]
        main_closes_i = main_closes[:i+1]
        main_highs_i = main_highs[:i+1]
        main_lows_i = main_lows[:i+1]
        main_volumes_i = main_volumes[:i+1]
        price = main_closes[i]

        confirm_idx = get_confirm_idx(i)
        if confirm_idx >= n_confirm:
            continue
        confirm_slice = confirm_ohlcv[:confirm_idx+1]
        confirm_closes_i = confirm_closes[:confirm_idx+1]
        confirm_highs_i = confirm_highs[:confirm_idx+1]
        confirm_lows_i = confirm_lows[:confirm_idx+1]
        confirm_price = confirm_closes[confirm_idx]

        # Compute indicators (using only data up to i)
        r = rsi(main_closes_i, 14)
        ema20 = ema(main_closes_i, 20)
        ema200 = ema200_main_series[i]
        trend_main = "UP" if price > ema20 else "DOWN"
        trend_ema200_main = "bullish" if price > ema200 else "bearish"

        macd_line = macd(main_closes_i, 12, 26)
        macd_bullish = macd_line > 0

        lower, middle, upper = bollinger_bands(main_closes_i, 20, 2)
        near_lower = price <= lower * 1.01
        near_upper = price >= upper * 0.99

        adx_val = adx(main_slice, 14)
        atr_val = atr(main_slice, 14)
        if atr_val is None:
            atr_val = price * 0.01

        avg_vol = sma(main_volumes_i, 20)
        volume_ratio = main_volumes[i] / avg_vol if avg_vol > 0 else 1
        volume_ok = volume_ratio >= min_vol_mult
        volume_spike = volume_ratio >= VOLUME_SPIKE_MULTIPLIER

        # Fake breakout
        avg_candle_size = sma([c['high'] - c['low'] for c in main_slice[-20:]], 20)
        current_candle_size = main_highs[i] - main_lows[i]
        ...
        # Market structure on main
        structure_main = get_market_structure(main_slice)

        # Confirmation indicators
        confirm_ema20 = ema(confirm_closes_i, 20)
        trend_confirm_dir = "UP" if confirm_price > confirm_ema20 else "DOWN"
        confirm_ema200 = ema200_confirm_series[confirm_idx]
        trend_ema200_confirm = "bullish" if confirm_price > confirm_ema200 else "bearish"
        structure_confirm = get_market_structure(confirm_slice)

        # BTC trend
        btc_trend = None
        if use_btc and btc_trends and confirm_idx < len(btc_trends):
            btc_trend = btc_trends[confirm_idx]
        ...
        if trend_ema200_main == "bullish" and trend_ema200_confirm == "bullish":
            conf += 20
        elif trend_ema200_main == "bearish" and trend_ema200_confirm == "bearish":
            conf += 20
        else:
            conf -= 10
        ...
        # Grade
        grade_score = 0
        if (signal == "BUY" and trend_confirm_dir == "UP") or (signal == "SELL" and trend_confirm_dir == "DOWN"):
            grade_score += 2
        elif signal != "HOLD":
            grade_score += 1
        ...
        if fake_breakout:
            continue
        if use_btc and btc_trend:
            if signal == "BUY" and btc_trend == "bearish":
                continue
            if signal == "SELL" and btc_trend == "bullish":
                continue

        # Enter trade if signal qualifies
        if signal in ("BUY", "SELL") and grade_score >= MIN_GRADE_SCORE:
            coin = symbol

            # Cooldown
            last_idx = last_trade_index.get(coin, -1)
            if last_idx != -1 and i - last_idx < cooldown_candles:
                continue
            ...
            # Global concurrent trades limit
            if len(open_trades) >= max_concurrent_trades:
                continue

            # Trades per hour limit
            one_hour_ago = i - (60 // main_minutes)
            trades_last_hour = sum(1 for t in trades if t['exit_index'] > one_hour_ago)
            ...
            # Exposure check
            total_exposure = sum(ot['entry_price'] * ot['quantity'] for ot in open_trades)
            if total_exposure + position_value > capital * max_exposure_percent / 100:
                continue
            ...
            last_trade_index[coin] = i
            logging.info(f"Entry {signal} {symbol} at {entry_price:.2f}, quantity {quantity:.4f}, SL {sl:.2f}, TP {tp:.2f}")

    # Close any remaining open trades at last price
    for ot in open_trades:
        if ot['remaining_quantity'] <= 0:
            continue
        exit_price = main_closes[-1]
        if ot['signal'] == "BUY":
            exit_price *= (1 - slippage)
        else:
            exit_price *= (1 + slippage)
        ...
        if ot['signal'] == "BUY":
            gross_pnl = (exit_price - ot['entry_price']) * ot['remaining_quantity']
        else:
            gross_pnl = (ot['entry_price'] - exit_price) * ot['remaining_quantity']

        entry_fee = ot['entry_price'] * ot['remaining_quantity'] * fee_percent
        exit_fee = exit_price * ot['remaining_quantity'] * fee_percent
        net_pnl = gross_pnl - entry_fee - exit_fee
        capital += net_pnl
...
# ================================
#  TELEGRAM SEND & COMMANDS (unchanged)
# ================================
...
def process_command(text):
    ...
    if cmd == "hi":
        send_message("👋 Production bot ready – final stability fixes applied.")
