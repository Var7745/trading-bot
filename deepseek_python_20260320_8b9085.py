...
- Fixed pair matching (BTCINR → BTCUSDT)
- Correct OHLCV endpoint (public.coindcx.com)
- Fixed analyze_coin_mtf() – added safe candle length check
...
# Use USDT pairs (actual API format)
...
# BTC trend filter uses USDT pair
BTC_SYMBOL = "BTCUSDT"
...
# ================================
#  FIXED: COINDCX DATA FETCH – CORRECT PAIR MATCHING & OHLCV ENDPOINT
# ================================
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                if isinstance(data, list) and data and all(isinstance(item, str) for item in data):
                    ...
                    logger.warning(f"Markets response not a list of strings (attempt {attempt})")
            else:
...
def get_pair_for_symbol(user_symbol):
    """
    Convert a user-provided symbol (e.g., 'BTCINR' or 'BTCUSDT' or 'BTC') to a valid CoinDCX USDT pair.
    Returns the pair string (e.g., 'BTCUSDT') or None if not found.
    """
    ...
    if key not in last_error_logged or now - last_error_logged[key] > 3600:
        logger.error(f"[PAIR NOT FOUND] {user_symbol}")
        last_error_logged[key] = now
    return None

def get_ohlcv(symbol, interval, limit=200):
    """
    Fetch OHLCV data for a symbol using the correct public endpoint.
    ...
    url = f"{COINDZX_PUBLIC}/market_data/candles"
    params = {
        "pair": pair,
        "interval": interval,
        "limit": limit
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                if isinstance(data, list):
                    ...
                    logger.warning(f"[OHLCV ERROR] {pair}: unexpected response format (attempt {attempt})")
            else:
                logger.warning(f"[OHLCV ERROR] {pair}: HTTP {resp.status_code} (attempt {attempt})")
        except Exception as e:
            logger.warning(f"[OHLCV ERROR] {pair}: {e} (attempt {attempt})")
        time.sleep(2)

    logger.error(f"[OHLCV FAILED] {pair} after 3 attempts")
    return None
...
# ================================
#  FIXED: ADVANCED ANALYSIS FUNCTION – ADDED SAFE CANDLE LENGTH CHECK
# ================================
def analyze_coin_mtf(user_symbol):
    ...
    if atr_percent > LEVERAGE_ATR_THRESHOLD:
        ...
        logger.info(f"{user_symbol} ATR% {atr_percent:.1f} > {LEVERAGE_ATR_THRESHOLD} – leverage capped to {leverage}")
    ...
    if use_btc and signal_main in ("BUY", "SELL"):
        ...
        if btc_trend:
            if signal_main == "BUY" and btc_trend == "bearish":
                logger.info(f"{user_symbol} BUY skipped: BTC trend bearish")
                return None
            if signal_main == "SELL" and btc_trend == "bullish":
                logger.info(f"{user_symbol} SELL skipped: BTC trend bullish")
                return None
    ...
    if use_dom and signal_main in ("BUY", "SELL"):
        ...
        if dom and dom > DOMINANCE_THRESHOLD and user_symbol != BTC_SYMBOL:
            logger.info(f"{user_symbol} skipped: BTC dominance {dom:.1f}% > {DOMINANCE_THRESHOLD}%")
            return None

    use_news = get_user_setting("use_news_filter", "false") == "true"
    if use_news and signal_main in ("BUY", "SELL"):
        if not get_news_sentiment():
            logger.info(f"{user_symbol} skipped due to negative news")
            return None
...
if __name__ == "__main__":
    ...
    logger.info("Starting CoinDCX Production Bot – FINAL VERSION with All Fixes")
