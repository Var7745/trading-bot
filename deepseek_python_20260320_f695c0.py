#!/usr/bin/env python3
"""
CoinDCX Ultimate Production Bot – FINAL VERSION with Correct Pair Matching
...
- Fixed pair matching for CoinDCX USDT pairs (e.g., BTCUSDT)
...
# FIXED: Use USDT pairs (actual API format)
...
# FIXED: BTC trend filter uses USDT pair
BTC_SYMBOL = "BTCUSDT"
...
# ================================
#  FIXED: COINDCX DATA FETCH – CORRECT PAIR MATCHING FOR USDT STRINGS
# ================================
markets_cache = {"timestamp": 0, "data": None}
pair_cache = {}          # in-memory cache for user_symbol -> pair
last_error_logged = {}   # rate-limited error logging

def get_markets():
    """
    Fetch the list of market symbols from CoinDCX.
    Returns a list of strings like ["BTCUSDT", "ETHUSDT", ...].
    ...
    fallback_list = ["BTCUSDT", "ETHUSDT", "XRPUSDT", "DOGEUSDT", "SOLUSDT"]
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                if isinstance(data, list) and data and all(isinstance(item, str) for item in data):
                    ...
                    logger.warning(f"Markets response not a list of strings: {type(data)} (attempt {attempt})")
            else:
    ...
    # After retries, return cached data if any, else fallback list
    if markets_cache["data"]:
        logger.error("Markets fetch failed, using cached data")
        return markets_cache["data"]
    logger.error("All markets fetch attempts failed, using fallback list")
    return fallback_list

def get_pair_for_symbol(user_symbol):
    """
    Convert a user-provided symbol (e.g., 'BTCINR' or 'BTCUSDT') to a valid CoinDCX pair.
    The actual API uses USDT pairs like 'BTCUSDT'.
    Returns the pair string or None if not found.
    """
    # Check in-memory cache first
    if user_symbol in pair_cache:
        return pair_cache[user_symbol]

    markets = get_markets()

    # 1. Direct match (if user sends USDT pair)
    if user_symbol in markets:
        pair_cache[user_symbol] = user_symbol
        logger.info(f"[PAIR FOUND] {user_symbol} (direct match)")
        return user_symbol

    # 2. If symbol ends with INR, convert to USDT (e.g., BTCINR -> BTCUSDT)
    if user_symbol.endswith("INR"):
        base = user_symbol.replace("INR", "")
        usdt_pair = f"{base}USDT"
        if usdt_pair in markets:
            pair_cache[user_symbol] = usdt_pair
            logger.info(f"[PAIR CONVERTED] {user_symbol} -> {usdt_pair}")
            return usdt_pair

    # 3. Try appending USDT (if user sent just base, e.g., "BTC")
    usdt_pair = f"{user_symbol}USDT"
    if usdt_pair in markets:
        pair_cache[user_symbol] = usdt_pair
        logger.info(f"[PAIR CONVERTED] {user_symbol} -> {usdt_pair}")
        return usdt_pair
...
def get_current_price(symbol):
    ...
    for attempt in range(1, 4):
        try:
            # CoinDCX ticker endpoint returns a dict keyed by pair
            ...
            logger.warning(f"Price fetch error for {pair}: {e} (attempt {attempt})")
        time.sleep(2)

    key = f"price_{pair}"
    now = time.time()
    if key not in last_error_logged or now - last_error_logged[key] > 3600:
        logger.error(f"All price fetch attempts failed for {pair}")
        last_error_logged[key] = now
    return None
...
# ================================
#  BTC TREND FILTER (using USDT pair)
# ================================
def get_btc_trend():
...
# ================================
#  ADVANCED ANALYSIS FUNCTION (updated to use pair)
# ================================
def analyze_coin_mtf(user_symbol):
    tf_main = get_timeframe()
    tf_confirm = "1h"

    # Convert user symbol to actual pair
    pair = get_pair_for_symbol(user_symbol)
    if not pair:
        return None

    # Use the pair for data fetching
    ohlcv_main = get_ohlcv(pair, tf_main, limit=200)
    ohlcv_confirm = get_ohlcv(pair, tf_confirm, limit=200)
    ...
    if current_candle_size > FAKE_BREAKOUT_MULTIPLIER * avg_candle_size:
        logger.info(f"{pair} fake breakout: candle too large")
        return None
    ...
    if atr_percent > max_atr_percent:
        logger.info(f"{pair} ATR% {atr_percent:.2f}% > {max_atr_percent}% – skipping")
        return None

    # Market regime filter
    if adx_val < MIN_ADX_FOR_TRADE:
        logger.info(f"{pair} ADX {adx_val:.1f} < {MIN_ADX_FOR_TRADE} – ranging market, skip")
        return None
    ...
    if atr_percent > LEVERAGE_ATR_THRESHOLD:
        ...
        logger.info(f"{pair} ATR% {atr_percent:.1f} > {LEVERAGE_ATR_THRESHOLD} – leverage capped to {leverage}")
    ...
    if use_btc and signal_main in ("BUY", "SELL"):
        ...
        if btc_trend:
            if signal_main == "BUY" and btc_trend == "bearish":
                logger.info(f"{pair} BUY skipped: BTC trend bearish")
                return None
            if signal_main == "SELL" and btc_trend == "bullish":
                logger.info(f"{pair} SELL skipped: BTC trend bullish")
                return None

    use_dom = get_user_setting("use_dominance_filter", "false") == "true"
    if use_dom and signal_main in ("BUY", "SELL"):
        dom = get_btc_dominance()
        if dom and dom > DOMINANCE_THRESHOLD and pair != BTC_SYMBOL:
            logger.info(f"{pair} skipped: BTC dominance {dom:.1f}% > {DOMINANCE_THRESHOLD}%")
            return None

    use_news = get_user_setting("use_news_filter", "false") == "true"
    if use_news and signal_main in ("BUY", "SELL"):
        if not get_news_sentiment():
            logger.info(f"{pair} skipped due to negative news")
            return None
    ...
    if capital > 0 and sl is not None:
        ...
        if position_size_inr < min_pos:
            logger.info(f"{user_symbol} position size ₹{position_size_inr:.2f} < min ₹{min_pos}, skipping")
            return None
    ...
    return {
        "symbol": user_symbol,  # Keep original user symbol for display
        "pair": pair,
        "price": price,
        "rsi": r,
...
- add BTCUSDT → Add a coin
- remove BTCUSDT → Remove a coin
...
📈 Analysis:
- analyze BTCUSDT → Get instant signal for a coin

🧪 Testing:
- backtest BTCUSDT 50 → Run backtest for 50 days
...
def process_command(text):
    ...
    # Original commands
        ...
        coin = parts[1].upper()
        # Allow both INR and USDT inputs; get_pair_for_symbol will convert
...
if __name__ == "__main__":
    ...
    logger.info("Starting CoinDCX Production Bot – FINAL VERSION with Correct Pair Matching")
